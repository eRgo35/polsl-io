...
public:
	...
	to rows and columns indices and . Lambda expressions are allowed.
	*/
	Matrix(int shape, double(*initializer_func)(int, int)); 

	/*Constructor initializing square matrix with values of a given vector.
	The data in a flat vector is used to fill the matrix row-wise, for example given a 2x5 matrix first five elements of the vector
	are written in the first row and five next elements are written into the second row.
	*/
	...
	//indices of a specific element. Returns False in case of error.
	bool load_data(double(*initializer_func)(int, int));

	// Prints elements of the matrix. Use only in TestingLab NO NEED TO TEST THAT.
	void print();
